 ...
 *
 * §6.1 defines canonical; ratified by both sides in v0.1.1 and unchanged since. It
 * was an undefined gap once, and a total one: when the two sides canonicalise
 * differently, <em>every</em> artifact is rejected. That is not hypothetical — it
 * happened over null-valued fields, and stayed invisible for as long as each side
 * only ever checked its own hasher against its own fixtures. See the inclusion note
 * on {@code canonical} below.
 *
 * <p>The ratified definition:
 *
 * <ul>
...
public class ContentHasher {
     ...
     *
     * <p><b>Inclusion is pinned to ALWAYS, and that is not optional.</b> This mapper is
     * a copy of the application's, which sets
     * {@code spring.jackson.default-property-inclusion: non_null} — right for API
     * responses, catastrophic here. Inherited, it silently drops null-valued fields from
     * the canonical form, so a body containing {@code "correlationId": null} hashes as
     * though the field were absent. §6.1 is "hash exactly what was sent", and a null
     * <em>is</em> content: absence is a different claim. The orchestrator draws the same
     * line in §7 — {@code null} counts as a present body.
     *
     * <p>This shipped undetected because our fixtures were generated by this class, so
     * they agreed with whatever it did — precisely the trap §6.1 names when it makes the
     * Q1 vectors, not the fixtures, the tiebreaker. It surfaced only when the
     * orchestrator started generating theirs from their own hasher and the two met over a
     * body with nulls in it. Any presentation-level Jackson setting reaching this mapper
